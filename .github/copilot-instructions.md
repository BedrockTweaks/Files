...
```
Files/
├── package.json                    # Root: Yarn 4 monorepo with workspaces: ["addons/files/*"]
├── addons/                         # Workspace root container
│   ├── packs.json                  # Addon catalog with versions
│   └── files/                      # Category container
│       ├── gameplay_changes/       # Category workspace
│       │   ├── package.json        # Category-level workspace declaring addons
│       │   └── graves/             # Individual addon package
│       │       ├── package.json    # Addon package: @bedrock-tweaks/graves
│       │       ├── config.json     # Regolith build config (TS → JS compilation)
│       │       ├── eslint.config.mjs  # ESLint config (TypeScript + Minecraft rules)
│       │       ├── packs/          # Output directory (generated by Regolith)
│       │       │   ├── BP/         # Behavior pack
│       │       │   └── RP/         # Resource pack
│       │       ├── data/gametests/src/main.ts  # TypeScript source
│       │       └── filters/        # Regolith filters (node-based only)
...
│   ├── files/                      # Organized by category → pack → assets
...
│   └── ISSUE_TEMPLATE/
├── .githooks/                      # Git hooks (pre-push linting)
...
1. **Root** (entry point): `yarn build` from root → runs `yarn workspaces foreach -i -A run build`
2. **Workspace discovery**: Root `package.json` declares `"workspaces": ["addons/files/*"]`, auto-discovering all addon categories
3. **Category-level workspaces**: Each category (e.g., `addons/files/gameplay_changes/`) has its own `package.json` with workspace declarations (e.g., `"workspaces": ["graves"]`)
4. **Each addon**: Executes its `package.json` build script: `regolith run build` (which compiles TS → JS)
5. **Output**: Generates `.mcpack` files in each addon's `build/` directory
6. **Shared dependencies**: All addons share a single `node_modules` at root (optimized for size/speed)
...
- `yarn regolith-install` - Install Regolith + dependencies for all addons (run once after `yarn install`)

**Key insight**: Yarn workspace discovery is **automatic and name-based**. Packages are identified by the `name` field in `package.json`, not by directory structure. The root workspace discovers category workspaces via glob pattern `addons/files/*`.
...
- **Linting**: Each addon uses `eslint.config.mjs` (flat config). Ignores include `.regolith/`, `.vscode/`, `**/*.js` (generated), `filters/`, and `build/`
- **Source files**: Only `data/gametests/src/` directory contains TypeScript; all others are generated or config
- **Plugins**: Use `@stylistic/eslint-plugin`, `typescript-eslint`, `@eslint/json`, and `eslint-plugin-minecraft-linting`
- **Code style**: 1 tab = 4 spaces (enforced), prefer `const`/`let`, interfaces over types, functional over OOP
- **No public functions**: Exclude player-facing UI openers to prevent operator access vulnerabilities
- **ESLint command**: Run `yarn run lint` from addon directory before committing
...
10. **Regolith watch mode**: Use `yarn run dev` (from addon dir) for live recompilation while testing in-game. Changes to `.ts` files auto-compile to `.js`.
11. **ESLint ignores generated files**: `**/*.js` is ignored by ESLint because all JS comes from TypeScript compilation—edit `.ts` files only.
12. **No Public Functions**: Script public functions can be called by any operator in-game, creating security risks. Use command handlers or UI buttons (private functions) instead.
13. **JSON format in packs**: Always 1 tab = 4 spaces. Nearest-neighbor interpolation for icon/texture scaling to preserve pixelation.
