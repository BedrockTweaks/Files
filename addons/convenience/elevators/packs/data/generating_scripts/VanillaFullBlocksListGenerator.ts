...
// ! This is the all the full blocks list which is in the current release version of the addon,
// ! we have to use this to make sure that newer full blocks will only be added at the end of the array
// ! to ensure backward compatibilities for the existing elevator blocks
// Recent update done at: 1.21.50
const currentVanillaFullBlocksList: string[] = [
...
world.afterEvents.playerSpawn.subscribe((playerSpawnEvent: PlayerSpawnAfterEvent): void => {
	...
	const { location: playerLocation, dimension }: Player = player;
	...
	for (const blockId of Object.values(MinecraftBlockTypes)) {
		if (excludeBlocksList.includes(blockId) || currentVanillaFullBlocksList.includes(blockId)) continue;

		if (exceptionBlocksList.includes(blockId)) {
			newVanillaFullBlocksList.push(blockId);
	...
	currentVanillaFullBlocksList.push(...newVanillaFullBlocksList);

	if (newVanillaFullBlocksList.length === 0) {
		console.warn('There are no new full blocks that can be added.');
	} else {
		console.warn(JSON.stringify(currentVanillaFullBlocksList));
	}
});
